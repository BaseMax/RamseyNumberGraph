...
###
#
# @Name : RamseyNumber/ramsey_5_5.py
# @Version : 1.0
# @Programmer : Max
# @Date : 2019-07-15
		...
		for i in range(art):
			for j in range(i+1, art):
				for l in range(j+1, art):
					for w in range(l+1, art):
						for e in range(w+1, art):
							items.append( list() )
							items[q].append( (i+1, j+1) )
							items[q].append( (i+1, l+1) )
							items[q].append( (i+1, w+1) )
							items[q].append( (i+1, e+1) )

							items[q].append( (j+1, l+1) )
							items[q].append( (j+1, w+1) )
							items[q].append( (j+1, e+1) )

							items[q].append( (l+1, w+1) )
							items[q].append( (l+1, e+1) )
							items[q].append( (w+1, e+1) )
							q+=1
					'''
					items.append( list() )
